...
export default {
    ...
    runIt() {
        ...
        const testCafeRunnerProcess = new Performer([
                ...
                `--speed`, `${preset.get(`speedFactor`).toString()}`,

                `${(preset.get(`thisColorMode`)) ? '--color' : '--no-color'}`,
                `${(preset.get(`thisDebugOnFail`)) ? '--debug-on-fail' : ' '}`,
                `${(preset.get(`thisSkipJSErrors`)) ? '--skip-js-errors' : ' '}`,
                `${(preset.get(`thisSkipUncaughtErrors`)) ? '--skip-uncaught-errors' : ' '}`,
                `${(preset.get(`thisStopOnFirstFail`)) ? '--stop-on-first-fail' : ' '}`,
                `${(preset.get(`thisQRCode`)) ? '--qr-code' : ' '}`,
                `${(preset.get(`thisDevMode`)) ? '--dev' : ' '}`,
                `${(preset.get(`thisLiveMode`)) ? '--live' : ' '}`,
            ]);
