...
export default {
    ...
    runIt() {
        ...
        const configAccessor = {
            ...
            speedFactor:
                atom.config
                    .get('testcafe-runner.testCafeOptions.speedFactor'),
            thisColorMode:
                atom.config
        ...
        const testCafeRunnerProcess = new TestCafeRunnerProcess([
                configAccessor.browsers.join(','),
                launchingScript.file.path,

                `--${scriptFormat.toString()}`, `"${scriptDeclaration.toString()}"`,

                `--hostname`, `${configAccessor.hostname.toString()}`,
                `--ports`, `${configAccessor.ports.toString()}`,
                `--speed`, `${configAccessor.speedFactor.toString()}`,

                `${(configAccessor.thisColorMode) ? '--color' : '--no-color'}`,
                `${(configAccessor.thisDebugOnFail) ? '--debug-on-fail' : ' '}`,
                `${(configAccessor.thisSkipJSErrors) ? '--skip-js-errors' : ' '}`,
                `${(configAccessor.thisSkipUncaughtErrors) ? '--skip-uncaught-errors' : ' '}`,
                `${(configAccessor.thisStopOnFirstFail) ? '--stop-on-first-fail' : ' '}`,
                `${(configAccessor.thisQRCode) ? '--qr-code' : ' '}`,
                `${(configAccessor.thisDevMode) ? '--dev' : ' '}`,
                `${(configAccessor.thisLiveMode) ? '--live' : ' '}`,
            ]);
