    ...
    parse(scriptLine) {
        ...
        try {
            if (editor.isBufferRowCommented()) {
                atom.notifications.addError(errors.lineIsComment, { detail: scriptLine });
                return false

            } else {
                const parsed = scriptLine.match(regexp);
                return {
                    format: parsed[1],
            ...
            atom.notifications.addError(errors.lineIsWrong, { detail: scriptLine });
            return false
        }
    }
}
